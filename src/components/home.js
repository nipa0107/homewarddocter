...
export default function Home() {
  ...
  const fetchDiagnosisCounts = () => {
    fetch("http://localhost:5000/diagnosis-count", {
      method: "GET",
      headers: {
    ...
    <main className="body">
      ...
      <div className="home_content">
        ...
        <div className="diagnosis-counts">
          <p className="count">จำนวนผู้ป่วยแยกโรค:</p>
          <ul>
            {diagnosisCounts.map((diagnosis) => (
              <li key={diagnosis._id}>
                {diagnosis._id}: {diagnosis.count} คน
              </li>
            ))}
          </ul>
        </div> */}
